...
def error_local(
    x0: jax.Array,
    xf: jax.Array,
    error_estimate: jtp.FloatLike | None = None,
    rtol: jtp.FloatLike = RTOL_DEFAULT,
    atol: jtp.FloatLike = ATOL_DEFAULT,
...
def heun_euler(
    ...
    *,
    ...
    tf_next_state: Callable[
        [State, State, Time, TimeStep], State
    ] = lambda x0, xf, t0, dt: xf,
) -> tuple[State, jax.Array, jax.Array, dict[str, Any]]:
    """"""
...
@functools.partial(
    ...
    static_argnames=[
        "f",
        "integrator_type",
        "implicit_quaternion_integration",
        "debug_buffers_size",
    ],
)
...
def _ode_integration_adaptive_template(
    ...
    *,
    odeint_adaptive_one_step: Callable[
        [State, Time, Time, TimeStep], tuple[State, dict[str, Any]]
    ],
